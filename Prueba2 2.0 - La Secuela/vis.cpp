...
int main(int argc,char *argv[])
{
   ...
   std::vector<std::vector<uint8_t>> imagerow(bmpfh.height);
    ...
    bmpfh.cpp = 24;
    bmpfh.offset = 54;
    bmpfh.Numberofcolors = 0;
    bmpfh.bmp_imp_colors = 0;

    std::ofstream out;
    out.open(argv[2],std::ios::out | std::ios::app |std::ios::binary);
    if (!out.is_open())
    {
    ...
    size_t rowsize2 = ((bmpfh.cpp * bmpfh.widht + 31) / 32) * 4;
    for (int i = 0; i < bmpfh.height; i++)
    {
        imagerow[i].resize(rowsize);
        out.write(reinterpret_cast<char*>(imagerow[i].data()),rowsize);

        if (in.gcount() != rowsize)
        {
            std::cerr << "Error writing BMP Pixel data\n";
            return 2;
        }
        
    }
    bmpfh.size = bmpfh.sizeh + bmpfh.size2 + imagerow.size();
    out.close();
